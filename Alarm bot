...
async def echo(ctx, *, minutes=None):  # Normalde echo aynı şeyi döndürmek için kullanılıyor ama ben böyle yapıp geçtim.
    ...
    try:
        ...
        while minuteint + an.minute > 60:
            ...
            an2 -= 60
        z = 0
        messagemention = await ctx.send(
            f"| {ctx.author.mention}")  # Büyük sunucularda kullanılabilmesi için bunu ekledim. Karışıklık olmasın diye.
        messagebiter = await ctx.send(
            f"| {minutes} dk kaldı.")  # Kaç dakika kaldığını takip edebiliyoruz. Dikkat dağıtıcı olmaması açısından saniye koymadım.
        while True:
            ...
            await messagebiter.edit(
                content=f"| {an3 + x - datetime.now().minute} dk kaldı.")  # Her dakikada 1 yazıyı editliyor.
